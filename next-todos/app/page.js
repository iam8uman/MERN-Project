...
const page = () => {
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center justify-center">
        <h1 className="m-5 p-2 text-5xl font-bold text-center">Next.js Todos</h1>
        <p className="text-center">Todo app built with Next.js</p>

        {/* input fields with add button  */}
        <div className="flex justify-center">
          <input
            className="max-w-screen-lg p-2 m-3 border-2 text-black border-gray-200 rounded-lg"
            type="text"
            placeholder="Add your todos here"
            onChange={(e) => setTodos(e.target.value)}
          />
          <button onClick={addTodo} className="w-full p-2 m-3 text-white bg-blue-500 rounded-lg">
            Add Todo
          </button>
          </div>
            ...
            <ul>
              {todoList.map((todo) => (
                <li className="flex justify-start">
                  <p className="p-2 m-3 w-full text-black bg-white rounded-lg">{todo}</p>
                  {/* delete Button  */}
                  <button className="p-2 m-3 text-white bg-red-500 rounded-lg">Delete</button>
                  {/* edit button  */}
                  <button className="p-2 m-3 text-white bg-green-500 rounded-lg">Edit</button>
                </li>
              ))}
